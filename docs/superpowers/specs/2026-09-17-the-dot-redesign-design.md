...
1. Arrival. At scroll 0 the visitor sees, left-aligned beside the dot: eyebrow, h1, supporting line, the button and the email link, the proof row, and a quiet "Scroll" cue with a 1px petrol line that grows on a 2s loop. The dot sits as the full stop after "works." in the headline. On the first scroll the text eases apart and the canonical mark blooms out of the dot with a `clip-path: circle()` reveal; the brand line fades in beneath. The cue fades within the first 10% and never returns.
...
5. Closing, 100svh. The mark settles once around the dot, then a compact block: "Talk to the founder" button, the legal line, and Work, About, Contact links. No second bloom sequence.
