...
export async function handleLead(req: Request, env: Env, f: typeof fetch = fetch): Promise<Response> {
  ...
  if (Number(req.headers.get('content-length') ?? 0) > MAX_BYTES) return json(413, { ok: false, message: 'Too large.' });
  ...
  if (Buffer.byteLength(raw) > MAX_BYTES) return json(413, { ok: false, message: 'Too large.' });
  ...
  try {
    ...
    if (!t.success || t.action !== 'lead' || !hosts.includes(t.hostname ?? '')) return json(400, { ok: false, message: 'Please complete the check and try again.' });
